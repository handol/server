...
PmsInetAddr::PmsInetAddr(const PmsInetAddr &sa) :
	PmsBaseAddr(sa)
{
	this->reset ();
	this->set (sa);
...
PmsInetAddr::PmsInetAddr(char *straddr, int portnum) // straddr : IP주소 문자열. "100.10.1.2"
{
	this->set(straddr, portnum);
}

PmsInetAddr::PmsInetAddr(unsigned int ipval, int portnum) // ipval : IP주소 4-byte value. Host order.
{
	this->reset();
	inet_addr_.sin_family = AF_INET;
...
PmsInetAddr::PmsInetAddr (const sockaddr_in *addr, int len)
{
	 this->set (addr, len);
}

void PmsInetAddr::reset(void)
{
	hostname[0] = 0;
	ipaddr[0] = 0;
...
void	PmsInetAddr::set (const PmsInetAddr &sa)
{
	if (sa.get_type () == -1)
...
// Creates a PmsInetAddr from a sockaddr_in structure.
void	PmsInetAddr::set (const sockaddr_in *_addr, int len)
{
	reset();
	memcpy (&this->inet_addr_, _addr, len);
}

void	PmsInetAddr::set(char	*straddr, int portnum)
{
	reset();	
	inet_addr_.sin_family = AF_INET;
	inet_addr_.sin_port = htons(portnum);
...
const char *PmsInetAddr::get_host_name(void) 
{
	if (hostname[0]==0) {
...
const char *PmsInetAddr::get_host_addr (void) 
{
	if (ipaddr[0]==0) {
...
*/
int	PmsInetAddr::_isIPnumber(char *straddr)
{
	char ch;
	while ( (ch=*straddr++) ) {
...
void PmsInetAddr::print(const char *msg)
{
	printf("PmsInetAddr(%s): type=%d, size=%d, addr.sin_family=%X, addr.sin_addr.s_addr=%X(%X), addr.sin_port=%X(%d)\n",
		(msg)?msg:"",
		addr_type_,
		addr_size_,
