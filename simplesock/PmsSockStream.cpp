...
int	PmsSockStream::open(int addrtype, bool reuseAddr)
{
	return PmsSock::open(SOCK_STREAM, addrtype, reuseAddr);
}

int	PmsSockStream::open(const PmsBaseAddr &addr, bool reuseAddr)
{
	int res = PmsSock::open(SOCK_STREAM, addr.get_type(), reuseAddr);
	if (res < 0)
		return -1;

	res =  PmsSock::bind(addr);
	if (res < 0) {
		printf("PmsSockStream: socket bind failed\n");
		return -2;
	}

	return this->sockfd;
}
int	PmsSockStream::close()
{
	#if defined (_WIN32)
...
*/
int	PmsSockStream::send(const void *buff, size_t size)
{
	int	ret;
#ifdef _WIN32
	...
	if (ret == SOCKET_ERROR) {
		...
		if (sock_errcode == EINPROGRESS || sock_errcode == ENOBUFS)
			...
			perror("PmsSockStream:: send() exeption");
			return -1;
		}
	}
	return ret;
...
int	PmsSockStream::send_n(const void *buff, size_t bytes_wanted, PmsTimeValue &timeout)
{
	int revents = 0;
	size_t sent_bytes=0;
	bool is_error = false;
...
int	PmsSockStream::recv(void *buff, size_t maxsize)
{
	int ret=0;
#ifdef _WIN32
	try {
	...
	{	
		...
		if (this->sock_errcode == EINPROGRESS)
			...
			perror("PmsSockStream:: recv() exeption");
			return -1;
		}
	}	
#else
...
*/
int	PmsSockStream::recv_n(void *buff, size_t bytes_wanted, PmsTimeValue &timeout)
{	
	int revents = 0;
	size_t read_bytes=0;
	bool is_error = false;
	...
	while(read_bytes < bytes_wanted)
	{
		...
		if (IS_POLL_R(revents)) {
			...
			//printf("PmsSockStream::recv_n - nread[%d] read_bytes[%d]\n", nread, read_bytes);
			if (nread > 0) { // 2010-6-3 
	...
	if (end_sec - start_sec >= 2) {
		printf("PmsSockStream::recv_n - delay[%d] sock[%d] timeout[%d] timeleft[%d] wanted[%d] read[%d] is_error[%d]\n",
