...
uint4 PmsSharedMem::DJBHash(const char * s)
{
	uint4 h = 5381;  // seed
	while (*s)		
...
key_t PmsSharedMem::ftok(const char *pathname, int proj_id)
{
...
// return 0 if fail.
key_t PmsSharedMem::path2key(const char shmpath_[])
{
#ifndef _WIN32
	// key file
	...
	return keyval;
#else
	key_t keyval = PmsSharedMem::ftok(shmpath_, 1);
	if (keyval == (key_t)-1) {
		printf("Error: file for SHM not exist: %s\n", shmpath_);
		return 0;
	}
...
// SHM 을 새로 생성
int PmsSharedMem::create(const char shmpath_[], const size_t shmsize_)
{	
	return this->create(PmsSharedMem::path2key(shmpath_), shmsize_);	
}

// SHM 을 새로 생성
int PmsSharedMem::create(const key_t keyval, const size_t shmsize_)
{
	this->shmkey = keyval;
...
int PmsSharedMem::open(const char shmpath_[], bool readonly)
{
	return this->open(PmsSharedMem::path2key(shmpath_), readonly);
}

// shmget and attach with keyval
int PmsSharedMem::open(const key_t keyval, bool readonly)
{
	this->shmkey = keyval;	
	if (this->shmkey == 0)
		return -1;
...
// attach
int PmsSharedMem::open(bool readonly)
{
	/// attach
#ifndef _WIN32
...
const char *PmsSharedMem::get_error()
{
	return 0;
}
