...
#include "PmsSockDgram.h"
#include "PmsLapTimer.h"
#include "PmsInetAddr.h"
#include "PmsUnixAddr.h"
#include "PmsPoll.h"

int	PmsSockDgram::open(int addrtype, bool reuseAddr)
{
	return PmsSock::open(SOCK_DGRAM, addrtype, reuseAddr);
}


int	PmsSockDgram::open(const PmsBaseAddr &addr, bool reuseAddr)
{
	int res = PmsSock::open(SOCK_DGRAM, addr.get_type(), reuseAddr);
	if (res < 0)
		return -1;

	res =  PmsSock::bind(addr);
	if (res < 0) {
		printf("PmsSockDgram: socket bind failed\n");
		return -2;
	}
...
*/
int	PmsSockDgram::send(const void *buff, size_t size, const PmsBaseAddr &addr)
{
	int	ret;
#ifdef _WIN32
	...
	if (ret == SOCKET_ERROR) {
		...
		if (sock_errcode == EINPROGRESS || sock_errcode == ENOBUFS)
			...
			perror("PmsSockDgram:: send() exeption");				
			return -1;
		}
	}
	return ret;
...
int	PmsSockDgram::send_n(const void *buff, size_t bytes_wanted, const PmsBaseAddr &addr, PmsTimeValue &timeout)
{
	unsigned int revents = 0;
	size_t sent_bytes=0;
	bool is_error = false;

	PmsTimeValue timeleft(timeout.msec());
	PmsLapTimer timer; // timer to check time-out.
	timer.start();
...
int	PmsSockDgram::recv(void *buff, size_t maxsize, PmsBaseAddr *client_addr)
{
	int ret=0;
	...
	if (client_addr != 0)
	{
		...
		if (client_addr->get_type() == AF_UNIX) {
			const PmsUnixAddr &unixaddr = dynamic_cast<const PmsUnixAddr &> (*client_addr);
			//printf("PmsSockConnector:: UNIX ADDR: %s\n", unixaddr.get_path());
		}
		else {
			PmsInetAddr &inetaddr = dynamic_cast<PmsInetAddr &> (*client_addr);			
			//printf("PmsSockDgram:: INET ADDR: %s : %d\n", inetaddr.get_host_addr(), inetaddr.get_port_number());
		}
		//printf("PmsSockDgram::recv .. fromlen = %d, sizeof(struct sockaddr) = %d\n",len,sizeof(struct sockaddr));
	}
	...
	if (ret < 0) {
		...
		if (this->sock_errcode == EINPROGRESS)
			...
			//perror("PmsSockDgram:: recv() exeption");
			return -1;
		}
	}
#else
	...
	if (addr.get_type() == AF_INET) {
		...
		PmsUnixAddr &unixaddr = dynamic_cast<PmsUnixAddr &> (addr);
...
*/
int	PmsSockDgram::recv_n(void *buff, size_t bytes_wanted, PmsBaseAddr *client_addr, PmsTimeValue &timeout)
{	
	unsigned int revents = 0;
	size_t read_bytes=0;
	bool is_error = false;
